...
def generate_prompt_midi(prompt: str, *, seed: int | None = None) -> PromptMidiResult:
    ...
    logger.info("generate_prompt_midi start", extra={"prompt": prompt, "seed": seed})
    ...
    try:
        notes_count = sum(len(p.ordered_notes) for t in tracks for p in t.patterns)
    except Exception:
        notes_count = None

    # If generator produced zero notes, apply deterministic fallback melody
    if notes_count == 0:
        logger.warning(
            "prompt_to_midi fallback triggered: zero notes generated",
            extra={"prompt": prompt, "seed": seed},
        )
        try:
            # Enriched deterministic fallback that infers key/mode from the prompt and
            # produces a small MIDI file using mido. If mido is unavailable, fall back
            # to the internal _fallback_melody implementation.
            def _infer_key_and_mode_from_prompt(local_prompt: str) -> tuple[int, str]:
                p = local_prompt.lower()
                if "minor" in p:
                    return 57, "minor"  # A minor
                if "dorian" in p:
                    return 62, "minor"  # D dorian-ish
                if "dark" in p:
                    return 57, "minor"
                if "bright" in p or "uplifting" in p:
                    return 60, "major"  # C major
                return 60, "major"

            def _build_scale(root_midi: int, mode: str) -> list[int]:
                KEY_PROFILES = {
                    "major": [0, 2, 4, 5, 7, 9, 11],
                    "minor": [0, 2, 3, 5, 7, 8, 10],
                }
                degrees = KEY_PROFILES.get(mode, KEY_PROFILES["major"])
                return [root_midi + d for d in degrees] + [root_midi + 12]

            def _generate_fallback_midi_bytes(local_prompt: str, seed_val: int = 0, bpm: int = 120) -> bytes:
                # Local imports to avoid adding module-level dependencies if not needed
                import io as _io
                import random as _random
                try:
                    from mido import Message as _Message, MidiFile as _MidiFile, MidiTrack as _MidiTrack, bpm2tempo as _bpm2tempo
                    from mido import MetaMessage as _MetaMessage  # for tempo
                except Exception:
                    # If mido isn't available, use the internal fallback that returns a Track
                    from app.api.audio.midi_generation import _fallback_melody as _internal_fallback

                    internal_track = _internal_fallback(
                        root_pitch=plan.engine_config.root_pitch if hasattr(plan.engine_config, "root_pitch") else 60,
                        scale=getattr(plan.engine_config, "scale", "major"),
                        bars=getattr(plan.engine_config, "bars", 2),
                        seed=seed,
                        tempo_bpm=plan.tempo_bpm,
                    )
                    return render_to_midi((internal_track,))

                _random.seed(seed_val)
                root, mode = _infer_key_and_mode_from_prompt(local_prompt)
                scale = _build_scale(root, mode)

                midi = _MidiFile()
                track_melody = _MidiTrack()
                midi.tracks.append(track_melody)

                tempo = _bpm2tempo(bpm)
                track_melody.append(_Message("program_change", program=0, time=0))
                track_melody.append(_MetaMessage("set_tempo", tempo=tempo, time=0))

                # deterministic rhythm + velocity patterns (ticks)
                rhythm_choices = [120, 240, 360]  # ticks-based durations (approx)
                velocity_base = 70

                time_acc = 0
                for i in range(8):
                    pitch = _random.choice(scale)
                    dur = _random.choice(rhythm_choices)
                    vel = velocity_base + (10 if i in (2, 5) else 0)

                    track_melody.append(_Message("note_on", note=pitch, velocity=vel, time=0))
                    track_melody.append(_Message("note_off", note=pitch, velocity=vel, time=dur))
                    time_acc += dur

                HARMONY_HINTS = ("chord", "chords", "harmony", "progression", "pad", "ambient")
                if any(h in local_prompt.lower() for h in HARMONY_HINTS):
                    track_bass = _MidiTrack()
                    midi.tracks.append(track_bass)
                    bass_root = root - 24
                    step = max(rhythm_choices)
                    t = 0
                    while t < time_acc:
                        track_bass.append(_Message("note_on", note=bass_root, velocity=velocity_base, time=0))
                        track_bass.append(_Message("note_off", note=bass_root, velocity=velocity_base, time=step))
                        t += step
            ...
            # Generate fallback MIDI bytes using prompt context
            midi_bytes = _generate_fallback_midi_bytes(prompt, seed if seed is not None else 0, plan.tempo_bpm)
            # Represent tracks as an empty tuple since bytes were rendered directly
            tracks = ()
        except Exception:
            logger.exception("fallback_melody generation failed; returning original MIDI bytes")

    logger.info(
        "generate_prompt_midi complete",
        extra={
            "prompt": prompt,
            "seed": seed,
            "pattern_type": parsed.pattern_type,
            "tracks": len(tracks),
            "midi_bytes_len": len(midi_bytes),
            "notes_count": notes_count,
        },
    )
    return PromptMidiResult(plan=plan, tracks=tracks, midi_bytes=midi_bytes)
